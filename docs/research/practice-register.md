...
| Public contract parity checks | accepted | V10 exact-candidate review and public-contract parity RCA | Map security- and lifecycle-significant ADR claims to every public summary and enforce prerequisites, non-guarantees, and Markdown placement with checker fixtures | Repeated green candidates drifted when secondary guides compressed the normative contract or rows rendered outside their intended table. |
