...
| Task-specific specialist compiler capability | accepted | Astraeus, provider custom-agent practices, V11 owner direction | Ship the repository IDE kickoff plus `docs/modules/specialist-agent-compiler.md`; compare candidate teams, expose pure profile-free `compileAgentBlueprints`, compile supply-free AgentBlueprints, and bind materialization receipts in V11 | Prevents vague role labels and false optimization claims by separating semantic intent, deterministic demand, runtime supply, host materialization, exact measurements, and reviewed baselines. |
...
| Public contract parity checks | accepted | V10 exact-candidate reviews, [CommonMark ownership snapshot](snapshots/2026-07-14-v10-commonmark-ownership-scan.md), and public-contract parity RCA | Bind significant claims to their intended paragraph or table row, require one active owner, and reject contradictory positive claims. For fenced Markdown, retain normalized quote/list state, marker-derived continuation columns, physical columns through stripped prefixes, partially consumed quote-padding tabs, container-relative fence indentation, context-matched closers, and surviving prefixes. Conservatively dispatch both explicit nested containers and continuation-only mixed horizontal whitespace to the rich parser, while keeping exact acceptance centralized there. Define blank as empty, spaces, or tabs; evaluate blankness relative to containers and end at the first unmarked quote. Pair every rejection with preserving coverage, keep raw source only for intentional literal checks, and retain independent semantic review. Keep V10 bounded and dependency-free; evaluate a proven parser or narrower contract separately if this surface expands. | Green CI repeatedly missed prose drift and parser bypasses, including container-relative blanks, U+00A0, physical-tab rebasing, quote-padding surplus loss, mixed fence indentation, continuation-only dispatch gaps, surviving-prefix loss, nested containers, arbitrary closers, fenced-only claims, duplicate owners, and host-newline assumptions. |
...
| Provider-neutral specialist runtime assignment | accepted | OpenAI Codex subagents and [2026-07-20 profile-routing scan](snapshots/2026-07-20-codex-subagent-profile-routing-scan.md) | Stage a post-V12 contract that derives runtime demand from each Specialist Agent Blueprint, matches it against a host-supplied capability inventory, and binds the reviewed selection to launch evidence | Lets an IDE choose an appropriately scoped model and effort while keeping specialist semantics portable, deterministic, inspectable, and owner-overridable. |
| `subagents_configs` Codex adapter reference | optional | `nsEytgXm/subagents_configs` and [2026-07-20 profile-routing scan](snapshots/2026-07-20-codex-subagent-profile-routing-scan.md) | Mine its static profiles and cost-aware routing rules for an optional Codex adapter; do not install or copy it into core | It demonstrates practical per-role model and effort configuration, but fixed Codex roles solve host configuration rather than task-specific decomposition and verified orchestration. |
| Opaque or self-modifying model router | rejected | V11 product boundary and [2026-07-20 profile-routing scan](snapshots/2026-07-20-codex-subagent-profile-routing-scan.md) | Permit measured runs to propose reviewed policy revisions only; never let history silently change runtime selection | Hidden routing would weaken reproducibility, approval identity, failure diagnosis, and the execution trace. |
