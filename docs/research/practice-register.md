...
| Public contract parity checks | accepted | V10 exact-candidate reviews, CommonMark 0.31.2, and public-contract parity RCA | Bind significant claims to their intended paragraph or table row, require an active unique owner, and reject contradictory positive claims. For fenced Markdown, retain normalized quote/list state and the longest surviving list prefix, derive continuation columns from marker width and normalize tab-expanded surplus, require a context-matched closer, and end an unclosed fence at its container boundary. Gate the top-level fast path on direct and indented nested-container ambiguity with equivalent rejection and preservation fixtures; keep raw source only for literal command examples and command or install claims. | Green CI repeatedly missed prose drift and parser bypasses, including surviving outer-container loss, tab continuation, indented nested-container fences, arbitrary closers, fenced-only claims, duplicate owners, and host-newline assumptions. |
