...
| Public contract parity checks | accepted | V10 exact-candidate reviews, CommonMark 0.31.2, and public-contract parity RCA | Bind significant claims to their intended paragraph or table row, require an active unique owner, and reject contradictory positive claims. For fenced Markdown, retain the opener's normalized quote/list stack, derive list continuation width from marker width plus following indentation, require a context-matched closer, and end an unclosed fence at its container boundary. Use an ambiguity-gated top-level fast path only with equivalent rejection and preservation fixtures; keep raw source only for literal command examples and command or install claims. | Green CI repeatedly missed prose drift and parser bypasses, including arbitrary container closers, multi-digit ordered-list continuation fences, fenced-only claims, newline-crossing headings, duplicate owners, and host-newline assumptions. |
