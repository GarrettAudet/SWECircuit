...
| Cooperative cancellation with abort_unconfirmed | accepted | MCP Tasks and Node process semantics | Before invocation, terminate safely when no executor work started; after invocation, require executor settlement or preserve potentially live work | Avoids false claims that remote, in-process, or descendant work stopped while keeping the no-call path explicit. |
