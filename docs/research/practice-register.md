...
| Public contract parity checks | accepted | V10 exact-candidate reviews and public-contract parity RCA | Bind security- and lifecycle-significant ADR claims to their intended paragraph or table row, scope nested owners, require keyed rows in the first contiguous table, normalize logical statements, and reject missing terms, contradictory positive claims, or misplaced Markdown with diagnostic-bound fixtures | Repeated green candidates drifted when nearby canonical prose, physical-line checks, or exit-only fixtures did not prove the rendered contract owner or causal rejection. |
