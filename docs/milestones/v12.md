...
V12 is not merge-ready. Candidate-addressed external evidence preserves every retired source and route. Revision 41 was retired after the committed lifecycle caught a dropped external TypeScript supply. Revision 42 fixed that behavior but its lifecycle identity preflight caught a stale byte count beside the new gate digest; no canonical gate ran. Revision 43 authenticated and executed, then exposed the paired host-dependency supply assumption. Revision 44 closes that supply explicitly. Frozen verification, one exact canonical gate, fresh R2, hosted CI, milestone closeout, and owner merge remain.
...
The product implementation remains integrated. Candidate-addressed evidence preserves Revision 40's gate and R2 outcomes plus every later pre-gate failure. Revision 44 is in verification with no release approval; its release-gate and release-review suites pass 53/53.
...
- Target branch: main.
- Current state: Revision 40 is retired after a passing gate and fresh-R2 security `fix`; Revision 41 is a causal correction with complete release-specific evidence.
...
Freeze Revision 41, run the complete verifier, and invoke its canonical gate exactly once. On `pass`, compile and launch fresh source-bound R2 reviewers across all three domains, then require hosted CI, milestone closeout, and owner merge evidence.

## User-Facing Overview

V12 provides the portable IDE run session, exact handoff verification, dependency routing, and integration-readiness behavior. Dogfooding has now caught ambient release authority and an incomplete independent-review context after a green gate. Revision 41 closes both boundaries and passes its release-specific suite; release still waits on the immutable gate, fresh R2, hosted CI, and merge.
