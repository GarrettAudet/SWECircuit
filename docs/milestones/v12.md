...
The V12 source on `codex/v12-ide-run-loop` carries working Revision 38 from released V11.1 baseline `c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c`. Revision 44 still passes the complete V11 trust replay, and exact Revision 35 aggregate/package-bound review evidence remains immutable. Revision 36 remains an exact 443/445 gate failure. Revision 37 `148f546cba4c3c9ceecd2bbca07d47fe94878afa` passed 446/446 plus lifecycle, package, and consumer gates, but its 57-source package reviewer returned verified `fix` for retained Git authority. Exact Revision 38 commit `1f2e89e30a46c1584cb7b979fc4c8a63326f7ff0` closes all inherited `GIT_*` authority and passes a clean 447/447 immutable aggregate. Its 69-source package reviewer returns exact verified `pass` and authorizes only an evidence-only Revision 39 successor. Candidate-addressed external evidence must still establish the successor's one-shot gate, fresh R2, hosted CI, milestone closeout, and owner merge. V12 is not merge-ready.
...
- Target branch: main.
- Current state: Revision 37 is an immutable `fix`; Revision 38 exact aggregate and package review pass with `successorFreezeApproved: true`; working Revision 39 still needs its one-shot gate, fresh R2, hosted CI, milestone closeout, and owner merge.
...
The external host still owns context delivery, runtime selection, spawning, isolation, persistence, integration effects, and merge. The distinct successor gate, fresh three-domain R2 review, and hosted CI can still expose a release-blocking issue. Automated model/effort assignment and IDE-native spawning are accepted post-V12 work, not V12 claims.

## Next Recommended Work

Commit the evidence-only Revision 39 successor and run its canonical gate exactly once. If it passes, launch three fresh source-bound R2 reviewers, then require hosted CI, milestone closeout, and owner merge evidence.

## User-Facing Overview

V12 provides the portable IDE run session, exact handoff verification, dependency routing, and integration-readiness behavior. Dogfooding caught both a parallel Git-context leak and an incomplete sanitizer that green tests missed. Revision 38 closes the boundary, passes its exact 447-test aggregate, and receives a verified 69-source independent `pass`. Revision 39 is the final evidence-only successor awaiting its one-shot release gate.