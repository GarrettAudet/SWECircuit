...
V12 is not merge-ready. Candidate-addressed external evidence preserves every retired source and route. Revision 46 externalized scratch and exposed one Windows shell-path canonicalization mismatch at R2 receipt validation. Revision 47 keeps strict binding, canonicalizes startup authority, passes 53/53 focused checks, and completes a direct copied-production lifecycle end to end. Frozen verification, one exact canonical gate, fresh R2, hosted CI, milestone closeout, and owner merge remain.
...
The product implementation remains integrated. Candidate-addressed evidence preserves Revision 40's gate and R2 outcomes plus every later pre-gate failure. Revision 47 is in verification with no release approval; its 53-test release suite and complete direct copied lifecycle pass.
