...
The product implementation is integrated. Candidates 4, 5, 6, and 7 are retired after exact gates exposed isolated Git-context, checkout-identity, runtime-state, host-toolchain supply, and isolated-test-default defects. Revision 14's exact package and handoff verify `pass`, V11 Revision 39 still replays exactly, and Candidate 8 pre-freeze verification passes. Candidate 8 freeze, its exact gate, and complete R2 review remain.
...
- Candidate 8 freeze, exact gate, fresh R2 review, hosted CI, and owner merge decision: pending.
...
The external host still owns context delivery, runtime selection, spawning, isolation, persistence, integration effects, and merge. Candidate 8 may still expose a release-blocking issue. Automated model/effort assignment and IDE-native spawning are accepted post-V12 work, not V12 claims.

## Next Recommended Work

Freeze Candidate 8, run its exact canonical gate once, and launch the three candidate-bound R2 reviewers only after that gate passes.

## User-Facing Overview

V12 makes an approved specialist package usable as a portable, restartable IDE run session with exact handoff verification and dependency readiness. Candidate 7 correctly exposed one isolated-test defect; Revision 14 closes it with verified evidence. Candidate 8's exact gate and one independent review wave remain before release approval.
