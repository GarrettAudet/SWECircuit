...
The V12 source on codex/v12-ide-run-loop carries Revision 31 from released V11.1 baseline c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c. Valid committed lifecycle evidence isolated false disposable-Git drift on deeply nested Windows candidate paths. The cause is confirmed: both disposable bare clones omitted core.longpaths. The bounded correction configures and verifies long-path support in both release contexts. Its full-tree causal regression, 29/29 release-review suite, and 16/16 release-gate suite pass. Candidate-addressed external evidence/receipts are authoritative for later gate state. V12 is not merge-ready.
...
The product implementation and prior causal corrections are integrated. Revision 31 has moved from diagnosis to correction verification: a 489-character same-context experiment proved the exact failure and counterfactual, both production constructors now require core.longpaths=true, the deep full-tree regression passes in 189.8 seconds, and production identities are rebound. The complete release-review and release-gate suites pass; the valid committed copied lifecycle, aggregate, and package-bound review remain next. Candidate-addressed external evidence/receipts alone may advance the exact successor gate, fresh R2, CI, and merge sequence.
...
- Target branch: main.
- Current state: the confirmed Windows long-path correction passes its focused causal test plus the complete 29-test release-review and 16-test release-gate suites; committed lifecycle, aggregate, package-bound review, an exact successor gate, fresh R2, hosted CI, and owner merge evidence remain required.
...
Run the release protocol in order: commit the exact correction, pass its valid committed lifecycle, preserve the aggregate and package-bound independent verdict, freeze one successor, execute its exact gate once, and launch three fresh source-bound R2 reviewers only from a passing candidate-addressed receipt.

## User-Facing Overview

V12 already provides the portable IDE run session, exact handoff verification, dependency routing, and integration-readiness behavior. Its final release harness exposed a real Windows deep-path defect; that cause is now fixed, and both complete source-level release suites pass. Release still requires the complete lifecycle, exact successor gate, independent R2 review, and hosted CI to pass before merge.
