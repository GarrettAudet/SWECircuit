...
V12 is not merge-ready. Candidate-addressed external evidence records Revision 60's exact local and hosted passes and its consumed one-shot canonical `fail`. Revision 61 closes the nested lifecycle Git-authority cause and passes focused, broad pre-freeze, checker, dogfood, package, consumer, and independent final-delta gates. Source-freeze eligibility is established; exact committed, hosted, canonical, fresh R2, closeout, and merge gates remain. `releaseReady: false`.
...
The product implementation remains integrated. Revision 60 is immutably retired after its canonical failure. Revision 61 is source-freeze eligible after causal runtime proof and final-delta review. No release approval exists until one immutable successor passes exact lifecycle, full verification, exact-candidate topology, hosted CI, one-shot canonical gating, and fresh three-domain R2.
...
- Revision 61 causal runtime, 65-test release pair, 462-test core, both checkers, build, dogfood, package, consumer, and independent final-delta review: `pass`; immutable source freeze remains.
...
- Target branch: `main`.
- Current state: Revision 61 focused, broad pre-freeze, and independent final-delta contracts pass; immutable source freeze is next.
...
Commit the exact Revision 61 bytes, then run copied lifecycle, full verification, exact-candidate topology, and the hosted matrix. Invoke the canonical gate exactly once. On `pass`, launch fresh source-bound R2 reviewers across all three domains, close the milestone and memory, and merge.

## User-Facing Overview

V12 provides a portable IDE run session with exact handoff verification, dependency routing, and integration-readiness evidence. Revision 61 hardens the release path so nested candidate verification keeps explicit Git authority and every executable regression is identity-bound and independently reviewable. Release remains pending exact immutable, hosted, canonical, fresh R2, and merge gates.
