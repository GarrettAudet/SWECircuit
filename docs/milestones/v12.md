...
V12 is not merge-ready. Revision 56's sole canonical gate at `6dfb99f86a88837ad94c2e73a422c0f6ba2d8c1f` returned `fail` after 457/458 core tests and is permanently retired; candidate-addressed external evidence preserves its exact receipt and logs. Revision 57 keeps the materialized tracked path above 260 characters while bounding source-repository and child-process working-directory paths; its nested-private-`TEMP` regression, causal selection, 24-test gate file, 61-test release contract, complete 458-test core suite, status invariants, template and quality gates, and independent review pass. Freeze and commit, exact committed lifecycle, full verification, one successor gate, fresh R2, hosted CI, milestone closeout, and merge remain; `releaseReady: false`.
...
The product implementation remains integrated. Revision 57 completed pre-freeze verification and independent review, and Revision 56's consumed gate remains immutable. No release approval exists until the distinct successor passes exact lifecycle, full verification, one-shot canonical gating, fresh three-domain R2, and hosted CI.
...
- Target branch: `main`.
- Current state: Revision 57's complete pre-freeze contract and independent review pass; exact source freeze and commit remain.
...
Freeze and commit Revision 57's exact bytes, then run the copied lifecycle and full verifier. Invoke the distinct canonical gate exactly once. On `pass`, launch fresh source-bound R2 reviewers across all three domains, require hosted CI, close the milestone, and merge.

## User-Facing Overview

V12 provides a portable IDE run session with exact handoff verification, dependency routing, and integration-readiness evidence. Revision 56's exact release gate found one Windows nested-`TEMP` path-budget flaw in its own long-path self-test; Revision 57 preserves the production guarantee with bounded fixture geometry and passes complete pre-freeze verification. Release remains pending the exact successor gates, independent R2, hosted CI, and merge.