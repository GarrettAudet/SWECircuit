...
V12 is not merge-ready. Candidate-addressed external evidence records that Revision 63 commit `7f45e75792caff01db638538004077b61643ea37` passed exact copied lifecycle and full verification plus all six hosted kernel jobs, but hosted run `30156840253` failed at tracked whitespace because exact raw evidence used non-exempt `.txt`. Its one-shot gate was never invoked and the source is retired. Revision 64 preserves the exact evidence bytes under the established tracked `.log` representation; focused, strict V11, template, checker-matrix, equivalent tracked-whitespace, complete verifier, and independent final-delta gates pass. Exact-candidate, hosted, canonical, fresh R2, closeout, and merge gates remain. `releaseReady: false`.
...
The product implementation remains integrated. Revision 60 remains retired after its consumed canonical failure. Revisions 61 through 63 are retired after unconsumed rehearsal, source-identity, and hosted-policy failures. Revision 64 has implemented the smallest representation correction and passed focused, broad, and independent final-delta verification. Independent Attempt 1's stale-state finding is integrated; final review reports no blocking or material finding. Exact lifecycle, full verification, non-consuming rehearsal, hosted CI, one-shot canonical gating, and fresh three-domain R2 remain.
...
- Target branch: `main`.
- Current state: Revision 64 focused, broad, and independent final-delta gates pass; exact source freeze is next.
...
The external host still owns context delivery, runtime selection, spawning, isolation, persistence, integration effects, and merge. Exact committed verification, hosted portability, canonical isolation, and fresh three-domain review can still expose a blocker. Automated model and effort assignment plus IDE-native spawning remain accepted post-V12 work, not V12 claims.

## Next Recommended Work

Freeze one exact Revision 64 commit. Require copied lifecycle, full verification, non-consuming exact-candidate rehearsal, and all seven hosted jobs before invoking the canonical gate exactly once. On `pass`, launch fresh source-bound R2 reviewers across all three domains, close the milestone and memory, and merge.

## User-Facing Overview

V12 provides a portable IDE run session with exact handoff verification, dependency routing, and integration-readiness evidence. Revision 64 preserves immutable release evidence under the repository's established hosted-log contract, and its focused, broad, and independent checks pass. Release remains pending exact immutable, hosted, canonical, fresh R2, and merge gates.