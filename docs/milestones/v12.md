...
The V12 source on `codex/v12-ide-run-loop` carries Revision 33 from released V11.1 baseline `c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c`. Revision 32 closed all Git-batch boundaries, but its exact aggregate stopped after 444/445 core tests when the final review-status edit omitted the required external-evidence delegation. Revision 33 corrects that source-only traceability regression. Candidate-addressed external evidence remains authoritative. V12 is not merge-ready.
...
- Target branch: main.
- Current state: Revision 33 passes post-edit anti-drift and both complete source-level release suites; a fresh exact aggregate, fresh package-bound review, one successor gate, fresh R2, hosted CI, and owner merge evidence remain required.
