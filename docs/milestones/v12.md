...
The V12 source on codex/v12-ide-run-loop carries Revision 31 from released V11.1 baseline c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c. The Windows long-path cause is fixed and both complete source suites pass. Committed checkpoint 8f1c4f1 crossed the former Git stop, then exposed a separate stable-identity error: unique parent temp paths entered the runtime binding and changed the package across phases. That cause is corrected, focused tests pass 3/3, release-review passes 30/30, and release-gate passes 16/16. Candidate-addressed external evidence/receipts are authoritative. V12 is not merge-ready.
...
The product implementation and prior corrections are integrated. Revision 31 now separates invocation-specific temp-path evidence from stable package identity while preserving exact worker validation. The second correction passes focused and complete source suites. A newly committed copied-production lifecycle is the next decisive gate; aggregate, package-bound review, exact successor gate, fresh R2, CI, and merge remain after it. Candidate-addressed external evidence/receipts alone may advance release state.
...
- Revision 31 stable-runtime correction: checkpoint 8f1c4f1 fails after 1,192.8 seconds at fresh approval identity; causal policy test 3/3 passes; complete release-review 30/30 passes in 80.3 seconds; complete release-gate 16/16 passes in 454.5 seconds.

## Approval Gate

- Source branch: codex/v12-ide-run-loop.
- Target branch: main.
- Current state: the long-path and stable-runtime identity corrections pass focused tests plus the complete 30-test release-review and 16-test release-gate suites; a newly committed lifecycle, aggregate, package-bound review, an exact successor gate, fresh R2, hosted CI, and owner merge evidence remain required.
