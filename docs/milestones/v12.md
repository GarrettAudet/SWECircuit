...
Successor-source freeze ready on `codex/v12-ide-run-loop` from released V11.1 baseline `c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c`: Revision 22, its independent review, V11 Revision 40 replay, the checker matrix, and the full pre-freeze gate pass; the most recent reviewed source remains retired, and V12 is not merge-ready.
...
The product implementation and Revision 22 trust-boundary correction are integrated. Historical source identities and outcomes remain preserved. The most recent committed source passed its one-shot exact canonical gate but is permanently retired because R2 security review found repository-live build output outside candidate authentication. Revision 22's implementation package, real copied-production lifecycle, independent review, refreshed V11 trust chain, checker matrix, and full pre-freeze gate pass. The integrated source is ready to freeze once.
...
- Target branch: `main`.
- Current state: implementation, Revision 22 correction, independent review, V11 Revision 40 replay, checker matrix, and full pre-freeze verification complete; exact release evidence pending; not merge-ready.
...
V12 makes an approved specialist package usable as a portable, restartable IDE run session with exact handoff verification and dependency readiness. Revision 22 proves the independent release-review runtime is candidate-derived, reconstructable, closed to host npm configuration, and exercised through the real lifecycle; the full pre-freeze gate also passes. The exact successor gate and fresh review remain; V12 is not merge-ready.
