...
mutable-source qualification and independent pre-freeze review pass. Copied lifecycle, complete
verifier, rehearsal, hosted Windows, canonical gate, fresh R2, milestone, and merge remain;
`releaseReady: false`.
...
Candidate-addressed external evidence preserves Revision 68's exact local passes and final hosted
matrix. ADR 0006 records the owner's Windows-only v0.1 support decision. Revision 69 is in
`clarify -> redesign -> verify`: the three-job CI contract, public support wording, bounded
compatibility correction, 49/49 release-review tests, 476/476 core tests, static checks, workflow
checker, and independent Windows-first rereview pass. Exact source freeze is next.
...
- Target branch: `main`.
- Current state: Revision 68 is retired; Revision 69 mutable-source qualification and independent
  Windows-first rereview pass. Immutable-candidate release gates remain.
...
persistence, integration effects, and merge. Revision 69 must pass Template Check plus Windows
...
Freeze one exact Revision 69 commit; require copied lifecycle, complete verification,
non-consuming rehearsal, Template Check, and Windows Node 22/24; then invoke its canonical gate
exactly once.
...
integration-readiness evidence. R68 proved the complete worker-authority binding locally, then
exposed one macOS process-environment portability gap. R69 targets that gap without accepting
arbitrary ambient variables; macOS is no longer a supported release host or confirmation gate under ADR 0006. Release remains pending exact immutable, hosted Windows, canonical, fresh R2,
and merge gates.
