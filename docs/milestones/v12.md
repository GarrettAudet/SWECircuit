...
qualifies frozen implementation source `a645f985dbb7b05f5b3656cd138968c456887568`; its immutable
mechanical gates and complete fresh R2 fan-in pass. This evidence-only closeout preserves the exact
source identity, canonical host chain, raw reviewer handoffs, verification report, residual host
boundary, and owner-approved merge decision.

## Acceptance

- AC1-AC8 remain checked in the immutable candidate spec; candidate-addressed closeout evidence
  satisfies AC9 for exact Revision 73 source `a645f985`.
- The canonical gate, raw streams, three independent R2 handoffs, and complete fan-in are preserved.
- Hosted Windows Template Check and Node 22/24 jobs pass in run `30239349407`.
- The owner approved the exact evidence-backed R73 release for merge to `main`.
...
- Revision 73 source candidate `a645f985dbb7b05f5b3656cd138968c456887568`: copied lifecycle 2/2, complete `npm.cmd run verify`
  with 487/487 core and 2/2 lifecycle tests, non-consuming rehearsal, hosted Windows run
  `30239349407`, and one-shot canonical gate all `pass`.
- Revision 73 canonical receipt/stdout/stderr: `sha256:0320921876a00df3d98c75afb55b8ef54cda5a79a234b4c44560c40801d40bde` /
...
- Fresh R2 compilation/package: `sha256:c53f05b73aafae0a39b08b4a2c09116ab59c9063c1d2dabb63486ea66aa83160` /
  `sha256:3dbd23419c4ae9e2dc0dac280f1d5727376c55afe81d0f2b437d0db1a50e5a2f`; exact outcomes `pass` / `pass` / `pass`; complete fan-in
  `releaseReady: true`.
...
- Target branch: `main`.
- Qualified Revision 73 source: `a645f985dbb7b05f5b3656cd138968c456887568`.
- Current state: all exact source, hosted Windows, canonical, and fresh R2 gates pass.
- Owner decision: approved for merge.
- Publication condition: fast-forward the R73 evidence closeout to `main` and confirm merged-main CI.
...
Revision 73 remains bounded by the external host: it owns context delivery,
provider/model/effort selection, agent spawning, workspace and process isolation, permission
enforcement, persistence, integration effects, merge, and memory mutation. Core verifies contracts,
packages, exact handoffs, dependency readiness, and immutable run state; it does not perform those
effects. V12 v0.1 supports Windows only. macOS and Linux qualification plus automatic runtime
assignment and IDE-native spawning remain post-V12 work.

## Next Recommended Work

Publish the approved Revision 73 closeout to `main` and confirm merged-main Windows CI. Then
dogfood R73 as the baseline for the next version: a provider-neutral host configuration and
runtime-assignment layer that maps each compiled specialist's declared demand to IDE-supplied
models, effort, skills, tools, workspaces, and native subagent launch.

## User-Facing Overview

Revision 73 makes V12 release-ready on Windows. Give the IDE a software goal, close the
specification and architecture, compile task-shaped specialist contracts, run dependency-safe work
through the external IDE host, verify exact handoffs, route failures visibly, and integrate only
after complete fan-in. The run session survives restart and preserves the execution trace without
locking the workflow to one IDE, model, or provider.
