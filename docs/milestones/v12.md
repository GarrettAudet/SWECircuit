...
V12 is not merge-ready. Candidate-addressed external evidence records Revision 39's exact passing gate and fresh-R2 `diagnose`. Working Revision 40 applies the bounded lineage-classification correction and passes focused plus 50/50 concurrent checks. Broad verification, an immutable successor, one exact canonical gate, fresh R2, hosted CI, milestone closeout, and owner merge remain.
...
The product implementation remains integrated. Candidate-addressed external evidence preserves Revision 39's gate `pass` and R2-preparation `diagnose` as distinct outcomes. Revision 40 is in verification with no release approval; focused and concurrent release checks pass.
...
- Target branch: main.
- Current state: Revision 39 is retired after a passing gate and fresh-R2 `diagnose`; Revision 40 is a causal correction with focused and concurrent evidence.
...
Complete Revision 40 broad verification, freeze a distinct commit, and run one exact canonical gate. On `pass`, compile and launch fresh source-bound R2 reviewers across all three domains, then require hosted CI, milestone closeout, and owner merge evidence.

## User-Facing Overview

V12 provides the portable IDE run session, exact handoff verification, dependency routing, and integration-readiness behavior. Dogfooding has now caught both runtime isolation defects and a final-review evidence-classification defect that green repository tests missed. Revision 40 closes that review boundary; release still waits on its immutable gate, fresh R2, hosted CI, and merge.
