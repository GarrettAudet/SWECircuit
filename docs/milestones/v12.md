...
The V12 source on `codex/v12-ide-run-loop` carries Revision 31's release-harness correction from released V11.1 baseline `c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c`. The latest canonical gate evidence preserved in this milestone records exact source integrity, 439 of 439 passing core tests, a copied-production lifecycle stop at the parent timeout/cleanup boundary, and source retirement. Revision 31 now rejects committed/live production-identity mismatches before lifecycle materialization. Candidate-addressed external evidence is authoritative for any later gate state. V12 is not merge-ready.
...
The product implementation and Revision 29 causal correction are integrated. Historical source identities and outcomes remain preserved. The latest source-preserved gate passed all 439 core tests, then exposed a copied-production timeout and cleanup-attribution defect before release review. Revision 31 batches exact Git object reads, scopes parent temp ownership, terminates timeout process trees, and reports process outcome before cleanup. Candidate-addressed external evidence alone advances the exact gate, R2, CI, and merge sequence.
