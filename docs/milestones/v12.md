...
V12 is not merge-ready. Revision 55's sole canonical gate at `7c0135c9604893f6baa933d123cd026323ee2912` returned `fail` after 454/458 core tests and is permanently retired; candidate-addressed external evidence preserves its exact receipt and logs. Revision 56 removes the four self-tests' ambient Git-discovery dependency without weakening production authority; its causal selection, 24-test gate file, 61-test release contract, 458-test core suite, template and quality gates, and independent review pass. Freeze and commit, exact committed lifecycle, full verification, one successor gate, fresh R2, hosted CI, milestone closeout, and merge remain; `releaseReady: false`.
...
The product implementation remains integrated. Revision 56 completed pre-freeze verification, and Revision 55's consumed gate remains immutable. No release approval exists until the distinct successor passes exact lifecycle, full verification, one-shot canonical gating, fresh three-domain R2, and hosted CI.
...
- Target branch: `main`.
- Current state: Revision 56's complete pre-freeze contract passes and its exact source identities are frozen for commit.
...
Complete Revision 56's independent and broad verification, freeze and commit its exact bytes, then run the copied lifecycle and full verifier. Invoke the distinct canonical gate exactly once. On `pass`, launch fresh source-bound R2 reviewers across all three domains, require hosted CI, close the milestone, and merge.

## User-Facing Overview

V12 provides a portable IDE run session with exact handoff verification, dependency routing, and integration-readiness evidence. The release gate exposed four tests that depended on checkout-local Git discovery; Revision 56 now isolates those tests while preserving the production security boundary. Release remains pending the exact successor gates, independent R2, hosted CI, and merge.