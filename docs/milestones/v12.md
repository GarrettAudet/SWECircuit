...
V12 is not merge-ready. Candidate-addressed external evidence records exact candidate state. Revision 64 commit `7d30a276d547cd501d93e6a698c84fff111bd8a4` passed exact standalone and all seven hosted checks, then failed the non-consuming exact-candidate rehearsal at the copied canonical gate's 30-minute bound. Its one-shot gate was never invoked and the source is permanently retired. Revision 65 preserves the exact evidence and applies the smallest nested Windows path-headroom correction; 71/71 focused release tests, every broad non-lifecycle gate, and independent final-delta review pass. Exact-candidate, hosted, canonical, fresh R2, closeout, and merge gates remain. `releaseReady: false`.
...
Candidate-addressed external evidence preserves Revision 64's exact standalone passes, all seven hosted successes, failed non-consuming rehearsal, and unused one-shot disposition. Revision 65 is in `fix -> verify`: its short private scratch namespace and causal regression are integrated, 71/71 focused release tests, every broad non-lifecycle gate, and independent final-delta review pass. Exact source freeze is next.
...
- Target branch: `main`.
- Current state: Revision 65 focused, broad non-lifecycle, and independent review gates pass.
...
The external host still owns context delivery, runtime selection, spawning, isolation, persistence, integration effects, and merge. Revision 65 still must prove its corrected path under exact materialization, hosted portability, one-shot canonical isolation, and fresh three-domain review. Automated model and effort assignment plus IDE-native spawning remain accepted post-V12 work, not V12 claims.

## Next Recommended Work

Freeze one exact Revision 65 commit, require copied lifecycle, complete verification, non-consuming rehearsal, and all seven hosted jobs, then invoke its canonical gate exactly once. On `pass`, launch fresh source-bound R2 reviewers, close milestone and memory, and merge.

## User-Facing Overview

V12 provides a portable IDE run session with exact handoff verification, dependency routing, and integration-readiness evidence. Revision 65 corrects the nested Windows verification path while preserving every product and host boundary. Focused, broad non-lifecycle, and independent gates pass; release remains pending exact immutable, hosted, canonical, fresh R2, and merge gates.
