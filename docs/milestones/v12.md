...
The V12 source on `codex/v12-ide-run-loop` carries working Revision 38 from released V11.1 baseline `c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c`. Revision 44 still passes the complete V11 trust replay, and exact Revision 35 aggregate/package-bound review evidence remains immutable. Revision 36 remains an exact 443/445 gate failure. Revision 37 `148f546cba4c3c9ceecd2bbca07d47fe94878afa` passed 446/446 plus lifecycle, package, and consumer gates, but its 57-source package reviewer returned verified `fix` for retained Git authority. Revision 38 removes all inherited `GIT_*` keys and passes 2/2 causal, 4/4 combined, 50/50 concurrent suites, the complete 447-test mutable-source verifier, the checker matrix, and strict V11 replay. Candidate-addressed external evidence must still establish a committed identity, fresh aggregate/package review, a distinct successor gate, fresh R2, hosted CI, milestone closeout, and owner merge. V12 is not merge-ready.
...
The product implementation remains integrated. Revision 36 is an immutable gate failure. Revision 37's aggregate passed, but its package-bound reviewer returned verified `fix` and retired it from release use. Revision 38 closes that authority boundary and passes causal, concurrent, broad mutable-source, checker-matrix, and strict V11 replay verification. A committed identity, fresh aggregate/package review, a distinct successor gate, fresh R2, hosted CI, milestone closeout, and merge remain.
...
- Revision 38 correction: causal 2/2 `pass`; combined contention 4/4 `pass`; authorized concurrent release suites 50/50 `pass`; complete mutable-source `npm.cmd run verify` `pass` in 679.3 seconds with 447/447 core tests, copied lifecycle, dogfood, package inspection, and offline consumer; checker matrix and strict V11 replay `pass`.

## Approval Gate

- Source branch: codex/v12-ide-run-loop.
- Target branch: main.
- Current state: Revision 37 package review is an immutable `fix`; Revision 38 causal, concurrent, broad mutable-source, checker, and V11 replay verification pass; committed identity, fresh aggregate/package review, a distinct successor gate, fresh R2, hosted CI, milestone closeout, and owner merge remain required.
...
The external host still owns context delivery, runtime selection, spawning, isolation, persistence, integration effects, and merge. Fresh aggregate/package review, the different successor gate, fresh three-domain R2 review, and hosted CI can still expose a release-blocking issue. Automated model/effort assignment and IDE-native spawning are accepted post-V12 work, not V12 claims.

## Next Recommended Work

Freeze Revision 38 as a committed identity, then pass a fresh aggregate plus newly compiled package-bound review. Preserve that passing evidence in a distinct successor before its one-shot gate. Only then launch three fresh source-bound R2 reviewers and require hosted CI, milestone closeout, and owner merge evidence.

## User-Facing Overview

V12 provides the portable IDE run session, exact handoff verification, dependency routing, and integration-readiness behavior. Dogfooding caught both a parallel Git-context leak and an incomplete sanitizer that green tests missed. Revision 38 closes the boundary and passes all 50 concurrent release tests plus the complete 447-test verifier; immutable aggregate and review evidence remain.