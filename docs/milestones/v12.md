...
V12 is not merge-ready. Revision 57 commit `e0859510132a5caf3a2393a5b203b07efef3d420` passed its exact copied lifecycle and full local verifier, then candidate-addressed external evidence from hosted run `30136535813` failed all seven jobs because Windows checkout lacked long-path bootstrap and depth-one checkouts omitted the approved historical checkpoint. Revision 57 is permanently retired without consuming its one-shot gate. Revision 58 supplies those repository prerequisites and passes focused and broad pre-freeze verification. Independent Attempt 3 review passes with no findings. Freeze and commit, exact committed lifecycle, full verifier, hosted CI, one successor gate, fresh R2, closeout, and merge remain; `releaseReady: false`.
...
The product implementation remains integrated. Revision 58 completed focused and broad pre-freeze verification, and Revision 57's local pass plus hosted failure remain immutable. No release approval exists until the frozen successor passes exact lifecycle, full verification, hosted CI, one-shot canonical gating, and fresh three-domain R2.
...
- Revision 58 focused regression, 63/63 release-specific tests, 460/460 core tests, checker matrix, template checker, format, lint, typecheck, and independent Attempt 3 review: `pass`; no pre-freeze finding remains.

## Approval Gate

- Source branch: `codex/v12-ide-run-loop`.
- Target branch: `main`.
- Current state: Revision 58's focused, broad, and independent pre-freeze contracts pass; exact source freeze and commit remain.
...
Freeze and commit Revision 58's exact bytes, then run the copied lifecycle, full verifier, and hosted matrix. Invoke the distinct canonical gate exactly once. On `pass`, launch fresh source-bound R2 reviewers across all three domains, close the milestone, and merge.

## User-Facing Overview

V12 provides a portable IDE run session with exact handoff verification, dependency routing, and integration-readiness evidence. Revision 57 proved the implementation locally, while hosted CI exposed missing repository-bootstrap declarations. Revision 58 makes Windows long paths and complete Git history explicit. Release remains pending the exact successor gates, independent R2, hosted CI, and merge.