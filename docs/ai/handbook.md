...
If compilation returns `SC4306`, call `analyzeSpecialistCandidates` with the exact reviewed request. Use its serial baseline, proposal evaluations, rejection codes, search claim, and bounded alternatives to revise the GoalContract. `selectionStatus: no_eligible_candidate` is diagnostic evidence, not a roster; never render or launch it.

Every generated `AgentBlueprint` is an exact task contract. A generic role label can never replace work ownership, context, authority, evidence, handoff, or stop conditions. `renderSpecialistPackage` returns `compilation.json`, `manifest.json`, `integration.md`, and one agent contract per blueprint, plus a root `packageDigest`. Each agent contract includes a concrete closed `SpecialistAgentHandoff` example bound to the exact goal, compilation, blueprint, destination, work, artifact set, evidence duties, and canonical workflow outcome. Agents replace the summary and string artifact contents while preserving that exact shape; they do not infer the handoff from the blueprint's different nested records.

Launch only after `verifySpecialistPackage` reproduces the package and matches trusted expected `compilationDigest` and `packageDigest` values. The launch approval record must bind both digests outside the package; values copied only from the untrusted package are not a trust decision. If an agent must semantically review the exact compilation that defines it, freeze that candidate and compile a separate read-only prelaunch audit over its complete compilation and rendered files. The owner-approved audit package is the explicit trust root; it does not recursively review itself and cannot launch or mutate the candidate. After the external host reconstructs the candidate and verifies the owner-approved audit package, it must create and preserve a closed `PrelaunchPackageVerificationReceipt` outside both packages before any audit agent launches. Deliver that receipt as a declared runtime input with `candidateLaunchApproved: false`. Before candidate approval, preserve a cross-package launch authorization binding both package digest pairs, the receipt's exact path, raw digest, byte count, and outcome, and the audit's exact raw semantic `PrelaunchAuditHandoff` path, digest, byte count, and outcome. The host must parse that closed envelope and validate its `pass` outcome, reviewer blueprint, goal, both digest pairs, completed review work unit, and both review-evidence duties; prose or an authorization flag alone is not approval.

The external IDE chooses runtime supply, verifies delivered context, enforces permissions and isolation, and executes only the exact approved contracts. Preserve each result as exact raw UTF-8 bytes and preserve the generated artifact media type exactly. Artifact content may use normalized LF but not TAB, CR, CRLF, or other unsafe controls. Run `verifySpecialistHandoff` against the approved package and digest pair; verification proves identity and closure, not success, so route any non-`pass` outcome normally.

Before a dependent integration blueprint starts, run `assessSpecialistHandoffs` over the exact raw handoffs from its complete transitive dependency closure. Continue only when the operation succeeds with a non-null value and `integrationReady: true`. Missing or valid non-`pass` handoffs keep readiness false; duplicate, unknown, outside-closure, malformed, stale, or substituted handoffs fail closed. Core returns immutable evidence and performs no runtime, persistence, merge, or memory effects.
