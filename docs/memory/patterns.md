...
Treat installed guides, schema READMEs, feature summaries, and governance tables as public contract surfaces. Maintain an ADR-to-surface matrix for security- and lifecycle-significant claims. Bind each invariant to one active, unique exact `##`/`###` paragraph owner or an exact keyed row in the first contiguous table. Use one active Markdown view for prose, navigation, headings, images, and semantic guards; bind fenced blocks to normalized quote/list context, retain the longest surviving list prefix when an inner container ends, and normalize tab-expanded continuation while rematerializing surplus fence indentation. An unmarked blank ends a quote-owned fence, while a quote-marked blank remains inside that container; preserve only enclosing list state before reprocessing. Inspect raw source only for intentional literal command examples and command-claim checks. A top-level fast path requires an ambiguity scan that includes indented nested-container fences plus equivalent rejection and preservation fixtures. Normalize logical statements across soft wraps, reject known opposite positive claims, require each negative fixture's intended diagnostic, and keep independent semantic review because lexical contradiction families remain bounded defense in depth.
