...
Treat installed guides, schema READMEs, feature summaries, and governance tables as public contract surfaces. Maintain an ADR-to-surface matrix for security- and lifecycle-significant claims. Bind each invariant to one active, unique exact `##`/`###` paragraph owner or an exact keyed row in the first contiguous table. Use one active Markdown view for prose, navigation, headings, images, and semantic guards; bind fenced blocks to normalized quote/list context, retain surviving container prefixes, preserve physical columns, rematerialize both continuation surplus and the unconsumed columns of quote-padding tabs, and normalize permitted fence indentation from the carried absolute column before delimiter matching. Use the grammar's exact space-or-tab blank class, walk container-relative blank state, and end ownership at the first unmarked quote while preserving the matched prefix. Inspect raw source only for intentional literal command examples and command-claim checks. A top-level fast path requires a conservative ambiguity scan that routes every plausible rich-parser syntax class, including continuation-only mixed horizontal whitespace when a list marker exists, plus equivalent rejection and preservation fixtures. Exact acceptance remains centralized in the rich parser. Normalize logical statements across soft wraps, reject known opposite positive claims, require each negative fixture's intended diagnostic, use invariant gate wording for current status, and keep independent semantic review because bounded lexical and structural checks are defense in depth rather than a full renderer.

### Grammar-Specific Blank State

When a checker models a text grammar, use that grammar's exact blank class rather than a host-language convenience predicate. Evaluate blankness relative to surviving containers, preserve only the matched prefix, and pair every causal rejection with a valid preservation case. Keep the implementation boundary explicit: bounded contract checking is not full parser conformance.

### Preserve Grammar Coordinates

When stripping prefixes in a coordinate-sensitive grammar, return both the remaining text and its physical coordinate. Tabs, indentation, and later nested parsing must continue from that coordinate; rebasing a substring to zero changes meaning. Pair the exact threshold rejection with a sufficient-indentation preservation case.

### Preserve Partially Consumed Columns

A delimiter can consume one virtual column from a source character that spans several columns. Preserve and rematerialize the unconsumed columns before parsing nested structure. Test both acceptance directions at the same threshold because whole-character deletion can hide active content or expose content that should remain inactive.

### Source-Derived Event Dates

For chronological memory, derive each event date from the event-owning Git, hosted-run, or other primary timestamp rather than session-start metadata. Use the repository working timezone for the compact date and preserve exact source timestamps when a date boundary matters. Keep genuinely older decision, issue-discovery, and snapshot dates unchanged.

### Candidate-Bound Verification Evidence

Bind every verification and review result to the immutable tree it exercised. A later tree may cite prior evidence for an unchanged surface, but it must name the owning candidate, label the result inherited, and keep exact-tree checks separate. Either rerun the evidence against the later tree or preserve the earlier ownership; do not blend the two into one current gate.

### Portable Policy, Adapter-Owned Effects

Keep goal decomposition, module composition, capability matching, dependency-aware fan-out and fan-in, gate policy, and trace semantics provider independent. IDE and provider adapters expose capabilities and perform assigned execution, isolation, persistence, and other side effects; they do not own the workflow policy. This lets one orchestration model operate across IDEs without reducing SWECircuit to API switching.
...
| [V10 public-contract parity reviews](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-public-contract-parity-review), [cross-surface matrix](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-cross-surface-settlement-review), [contradiction review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-contradiction-and-acceptance-state-review), [logical Markdown review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-logical-markdown-ownership-review), [active Markdown ownership review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-active-markdown-ownership-review), [container and heading ownership review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-container-and-heading-ownership-review), [active README review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-active-readme-and-line-boundary-review), [container-context review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-container-context-review), [nested-container review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-nested-container-and-tab-review), [blank-separated quote review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-blank-separated-blockquote-review), [container-relative blank review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-container-relative-blank-and-unicode-review), [absolute-column review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-absolute-column-and-timing-review), [quote-padding tab review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-quote-padding-tab-review), [mixed fence-indentation review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-mixed-fence-indentation-review), [nested ambiguity-gate review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-nested-ambiguity-gate-review), [mixed continuation dispatch review](../specs/v10-executor-adapter/debug-notes.md#exact-candidate-mixed-continuation-dispatch-review), [CommonMark snapshot](../research/snapshots/2026-07-14-v10-commonmark-ownership-scan.md), and [latest RCA](../specs/v10-executor-adapter/root-cause-analysis.md#mixed-list-continuation-dispatch-addendum) | Public Contract Surface Parity; Grammar-Specific Blank State; Preserve Grammar Coordinates; Preserve Partially Consumed Columns; Preserve Container-Relative Fence Indentation; Conservative Parser Dispatch |
...
| [V10 milestone](../milestones/v10.md#next-recommended-work) and [decision](decisions.md) | Portable Policy, Adapter-Owned Effects |
