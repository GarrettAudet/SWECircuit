...
Treat installed guides, schema READMEs, feature summaries, and governance tables as public contract surfaces. Maintain an ADR-to-surface matrix for security- and lifecycle-significant claims. Bind each invariant to one intended active paragraph or table row, require its prerequisites and non-guarantees there, and reject known opposite positive claims even when canonical prose remains elsewhere in the section. Verify structural placement as well as text presence; a truthful phrase outside its intended contract location is still defective.
