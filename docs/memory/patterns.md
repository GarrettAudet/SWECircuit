...
Treat installed guides, schema READMEs, feature summaries, and governance tables as public contract surfaces. Maintain an ADR-to-surface matrix for security- and lifecycle-significant claims. Bind each invariant to one active, unique exact `##`/`###` paragraph owner or an exact keyed row in the first contiguous table; strip fenced code before structural lookup and require prerequisites and non-guarantees in that owner. Normalize logical statements across soft wraps, reject known opposite positive claims, preserve truthful-negative acceptance cases, and require each negative fixture's intended diagnostic. Lexical contradiction families are bounded defense in depth, so independent semantic review remains required.
