...
Treat installed guides, schema READMEs, feature summaries, and governance tables as public contract surfaces. Maintain an ADR-to-surface matrix for security- and lifecycle-significant claims. Bind each invariant to one active, unique exact `##`/`###` paragraph owner or an exact keyed row in the first contiguous table. Use one container-aware active Markdown view for prose, navigation, headings, images, and semantic guards; inspect raw source only for intentional literal command examples and command-claim checks. Limit structural fence indentation, use horizontal whitespace plus explicit optional carriage returns for line owners, normalize logical statements across soft wraps, reject known opposite positive claims, preserve truthful-negative and fenced-example acceptance cases, require each negative fixture's intended diagnostic, and make fixture matching CRLF/LF-neutral. Lexical contradiction families are bounded defense in depth, so independent semantic review remains required.
