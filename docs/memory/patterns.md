...
Treat installed guides, schema READMEs, feature summaries, and governance tables as public contract surfaces. Maintain an ADR-to-surface matrix for security- and lifecycle-significant claims. Bind each invariant to one active, unique exact `##`/`###` paragraph owner or an exact keyed row in the first contiguous table. Use one container-aware active Markdown view for every structural lookup, including required H1/H2 ownership and fences nested under blockquotes or lists; inspect raw source only for intentional fenced examples. Normalize logical statements across soft wraps, reject known opposite positive claims, preserve truthful-negative acceptance cases, require each negative fixture's intended diagnostic, and make fixture matching CRLF/LF-neutral. Lexical contradiction families are bounded defense in depth, so independent semantic review remains required.
