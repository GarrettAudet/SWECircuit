...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `b3ff0d3` passed all seven jobs in run `29370427573` plus API/documentation review; correctness and security returned `REVISE` for fenced-content ownership, and correctness also found duplicate exact heading owners. The current correction strips backtick and tilde fences before structural parsing, requires one exact section or subsection owner, retains first-table-row and logical-statement checks, and binds fixtures to expected diagnostics. The positive checker and all 71 scenarios pass: 67 expected rejections, four expected acceptances, and 28 parity cases. The current acceptance state awaits exact-commit review, hosted CI, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
