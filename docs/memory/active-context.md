...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `82c3bb1` passed all seven jobs in run `29390051639` in 10m21s; exact review returned correctness and API/documentation `PASS` plus security `REVISE` because mixed space-plus-tab fence indentation was measured in columns but matched as raw characters. The current correction normalizes only the final zero-through-three fence-indentation columns at the carried absolute position for both openers and closers. The complete 112-scenario harness passes in 554.2 seconds: 96 expected rejections, 16 expected acceptances, and 30 unchanged executor contract-parity cases. Three direct causal probes, the positive checker, and `npm.cmd run verify` also pass. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
