...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `f990abc` passed all seven jobs in run `29384351025` in 6m25s, but correctness, security, and API/documentation each returned `REVISE`. Correctness found container-relative blank state loss, security found .NET Unicode whitespace broader than CommonMark blank syntax, and API/documentation found one current-versus-historical attribution ambiguity. The current correction uses exact space-or-tab blank semantics, preserves every surviving container before the first unmarked quote, and retains paired rejection and preservation coverage. The complete 103-scenario harness passes: the corrected run completed in 493.8 seconds and the final exact-tree rerun completed in 487.6 seconds, with 91 expected rejections, 12 expected acceptances, and 30 unchanged executor contract-parity cases. Direct causal and preserving probes plus the positive checker also pass. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
