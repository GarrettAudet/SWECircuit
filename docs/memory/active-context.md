...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Candidate `e3453e0` passed hosted CI but returned `REVISE / PASS / REVISE`. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and GitHub Actions run `29357443883`. The first closeout phrase search found one stale spec sentence. Docs-only candidate `2c6dff4` corrected it and passed run `29358105210`, but expanded exact review returned `PASS / REVISE / REVISE` for synonymous plan and practice-register claims. The complete claim family is now corrected. One final exact commit, checker replay, review, hosted CI, evidence-only closeout, and owner approval remain. V10 is not merged, and `main` remains the V9 baseline.
...
- Freeze, push, and run exact checks on the complete docs-only V10 claim-family correction.
