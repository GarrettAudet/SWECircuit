...
V12 IDE Run Loop is in Revision 28 release hardening on `codex/v12-ide-run-loop` from released V11.1 baseline `c2f974d2288fc510cb8388fbc8e6abe9fd5d9e8c`. Historical attempts and immutable evidence remain preserved. Revision 27 passed the full aggregate gate in 45 minutes 43 seconds, within its 46-50 minute forecast, but a subsequent immutable package-bound review correctly emitted `fix`: the packed-consumer gate reduced an authenticated TypeScript binding to a path, its PATH sentinels were not platform-selectable commands, and the documented boundary needed to distinguish entrypoint checks from host-enforced concurrent-writer and transitive toolchain isolation. Revision 28 preserves the full binding and receipt through `executeTypeScript`, exercises selectable PATH conflicts and persistent compiler mutation, and states the host boundary explicitly. Its focused toolchain, installed-consumer, V12 gate, copied-production review, format, lint, typecheck, build, whitespace, and complete immutable lifecycle gates pass. The lifecycle bound checkpoint `49ed27c859a6530ccaf69fcf53e6b2f418053f15` and passed in 38 minutes 4.6 seconds. Candidate 13 remains unconsumed. V12 is not merge-ready pending V11 evidence replay, aggregate verification, independent package-bound `pass`, one exact Candidate 13 gate, fresh R2 review, hosted CI, milestone closeout, and owner merge.

Revision 41 binds Candidate A `sha256:4394f9bbc2be8871b1b04f1b3b8202b43928251bbd8b996cc9cdeb9d5c84c055` / `sha256:9eea6f0084c5705d448aba40a50d1fb42401a1a8d6be53f847751f6c55a4bde1` and Audit B `sha256:fa7f65178897bae642e165df81fdfb0484e004ab942a9217c0ed1f7baae74371` / `sha256:b9e26224028a040da07780f747ba752174a8e424603c03981dd1abaffda85598`. The first binder correctly returned `block` because the host had generated but not explicitly delivered the external receipt; its exact 5,295-byte handoff is preserved. After the host delivered the exact 2,255-byte receipt as a declared runtime input, the 6,862-byte binder, 7,691-byte independent semantic `pass`, 984-byte canonical authorization, and `--check-evidence` replay all pass. V12 dogfood, the 148-file package dry run, and the clean offline installed-consumer gate also pass. Durable rule: review tooling is candidate evidence, every declared source must remain retrievable, runtime inputs must be explicitly delivered rather than inferred from repository presence, real production lifecycle proof must supplement projections, and closed package-manager configuration requires distinct private files rather than null-device aliasing or host fallback.

## Current Stage

V11.1 remains the released public baseline on `main`. V12 is the active dogfood version at the Revision 28 V11 evidence-replay gate after its immutable lifecycle passed. The next route is fresh V11 source-bound evidence, aggregate verification, and a package-bound independent review before Candidate 13 may be frozen. After that candidate passes its one-shot canonical gate, V12 still requires all three independent R2 reviewers, hosted CI, milestone closeout, and the owner merge gate. V12 remains provider-neutral: the external IDE owns concrete model/effort selection, skills, tools, spawning, isolation, execution, persistence, integration effects, and merge.
...
- PATH-conflict tests must create commands the target platform can actually select by the invoked command name.
