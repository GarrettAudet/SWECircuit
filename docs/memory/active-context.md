...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `ac70efc` passed all seven jobs in run `29364033724` and correctness review; security and API/documentation returned `REVISE` for presence-only parity enforcement and candidate-dependent status prose. Both findings now have causal corrections. The fresh canonical gate passes format, lint, typecheck, build, 275 tests, deterministic V10 dogfood, package inspection, and the clean offline consumer; the positive checker and all 62 scenarios also pass. The current acceptance state awaits exact-commit review, hosted CI, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
