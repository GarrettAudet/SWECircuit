...
Dogfood the technically accepted V10 contracts to design V11 as an IDE-, API-, model-, and provider-agnostic software-work coordinator. The target is one resumable goal-to-evidence facade that requires one supplied allowlisted profile, defaults requested concurrency to one, and scales only through explicit bounded concurrency plus host-authored replication and specialist capacity. Keep V10 immutable behind its owner gate and V11 planning-only until one later architecture candidate passes independent review.
