...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `0f952d9` passed all seven jobs in run `29386833535` in 9m19s; exact review returned correctness `PASS`, security `REVISE` for lost absolute tab columns after quote stripping, and API/documentation `REVISE` for inconsistent timing provenance. The current correction preserves physical columns across quote and list prefixes, rematerializes surplus indentation at the correct coordinate, and retains paired rejection and preservation coverage. The complete 105-scenario harness passes in 483.7 seconds: 92 expected rejections, 13 expected acceptances, and 30 unchanged executor contract-parity cases. Direct causal and preserving probes plus the positive checker also pass. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
