...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `ae5195c` passed all seven jobs in run `29383056180`; API/documentation returned `PASS`, correctness returned `REVISE`, and security produced no verdict within two bounded waits plus an immediate-conclusion window. Correctness proved that an unmarked blank ended a quote container while the parser incorrectly kept its quote-owned fence open, hiding a later active overclaim. The current correction ends quote-owned fences on unmarked blanks, preserves only an enclosing list prefix, and reprocesses the blank through normal list state. The complete 99-scenario harness passes in 440.7 seconds: 89 expected rejections, ten expected acceptances, and 30 unchanged executor contract-parity cases. Direct causal and preserving probes plus the positive checker also pass. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
