...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `57bab44` passed all seven hosted jobs in run `29396782369` in 11m35s and security review, but correctness and API/documentation returned `REVISE`: several summaries blended the 119-scenario matrix owned by `7f30107` with exact-tree checks run for `57bab44`. The evidence-attribution correction names `7f30107` as the owner of the latest complete matrix and reserves current-candidate claims for checks actually run on that candidate. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
...
- SWECircuit's target orchestration layer owns portable goal decomposition, capability matching, dependency-aware fan-out and fan-in, gates, and trace semantics; IDE and provider adapters perform assigned execution and side effects without owning workflow policy.
