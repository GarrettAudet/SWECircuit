...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `c4bfa01` passed all seven jobs in run `29380939276` but correctness, security, and API/documentation all returned `REVISE`: implicit inner-container termination discarded a surviving outer list, the ambiguity gate missed indented nested containers, tab expansion could overshoot exact continuation width, one retired-URL guard remained raw, and one next action self-staled. The current correction retains the longest surviving list prefix, normalizes tab-expanded indentation and rematerializes surplus columns, routes indented nested-container fence signatures to the rich parser, and keeps README identity guards on active Markdown. The complete 96-scenario harness passes in 345.8 seconds: 87 expected rejections, nine expected acceptances, and 30 unchanged executor contract-parity cases. Direct causal and preserving probes plus the positive checker also pass. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
