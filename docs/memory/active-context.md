...
V10 implements one provider-neutral, host-injected work-packet executor with an invocation-scoped grant and caller-owned V9-compatible journal. Runtime candidate `9d8907a` passed 275 tests, all three exact reviews, and run `29357443883`. Later closeout candidates exposed lifecycle, authority, and public-summary drift. Candidate `49b22ba` passed all seven jobs in run `29393468684` in 11m56s, but exact correctness, security, and API/documentation review all returned `REVISE`: continuation-only mixed space-tab fences bypassed rich-parser dispatch, and one next-action record self-staled after candidate creation. The current correction conservatively routes any horizontally indented potential fence to the coordinate-aware rich parser when a list marker exists while retaining exact parser acceptance. Three direct probes, the positive checker, `npm.cmd run verify`, and the complete 119-scenario harness pass in 576.2 seconds: 100 expected rejections, 19 expected acceptances, and 30 unchanged executor contract-parity cases. The acceptance gate remains exact-commit review, all seven hosted jobs, evidence-only closeout, and owner approval. V10 is not merged, and `main` remains the V9 baseline.
