...
| Astraeus | optional-adapter | Policy-compiler implementation for synthesizing project-specific subagents, critic path, synthesis path, and scoped permissions under SWECircuit's portable orchestration contracts. | A project needs repeatable agent-team synthesis beyond static decomposition templates. | It may compile SWECircuit-owned policy but is not an IDE, model, or provider execution adapter; install only after adapter evaluation. |
