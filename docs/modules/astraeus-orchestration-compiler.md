...
Astraeus is an optional policy-compiler implementation of SWECircuit's built-in `specialist-agent-compiler.md` and portable orchestration contracts. It may help synthesize closed GoalContracts, atomic work units, and candidate partitions for the built-in `compileAgentBlueprints` operation, but it does not own workflow semantics and is not an IDE, model, or provider execution adapter. SWECircuit ships the contract by default and installs nothing.
