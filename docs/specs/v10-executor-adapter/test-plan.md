...
- Candidate `4c6818d` passed run `29359564312` but returned `REVISE / PASS / REVISE` for per-call grant wording, a missing packaged executor-promise precondition, and two practice rows outside the register table. The corrected working tree passed `npm.cmd run verify` with 275 tests, the positive checker, and all 43 negative scenarios; exact-commit review and hosted CI remain.
