...
- The first pre-closeout phrase search found one stale no-call requirement in the feature spec. Candidate `2c6dff4` corrected it and passed hosted CI, but expanded review returned `PASS / REVISE / REVISE` for synonymous plan and practice-register claims. The complete correction requires the positive and 43-scenario checker replay, exact review, and hosted CI before acceptance.