...
- The current parity correction passes the positive checker and all 62 isolated scenarios: 59 expected rejections and three expected acceptances. Nineteen parity cases cover missing terms, structural relocation, contradictory claims, and register placement. Fresh `npm.cmd run verify` passes 275 tests, deterministic V10 dogfood, package inspection, and the clean offline consumer. Exact-commit review and hosted CI remain.
