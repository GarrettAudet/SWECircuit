...
## Container Context And List Continuation Addendum

Candidate `7f02b87` passed all seven hosted jobs in run `29377581706`, but all three exact reviewers returned `REVISE`. The fence parser recognized delimiters without preserving their block-container identity: it missed valid continuation indentation for multi-digit list markers, accepted unrelated container-prefixed closers, and failed to end a nested fence when its owning container ended.

CommonMark 0.31.2 establishes the causal model: fenced blocks are interpreted within block containers, and list continuation indentation derives from the marker width plus following indentation. The correction tracks a normalized quote/list stack, computes marker-derived continuation widths, closes only in the matching container, ends a nested fence at container termination, and reprocesses the outer line. An ambiguity gate keeps ordinary top-level fences on the simple parser while container-sensitive documents use the full state machine.

Five new fixtures prove the causal boundary and preservation behavior. The expanded harness passes 89 scenarios: 82 expected rejections, seven expected acceptances, and 30 unchanged executor contract-parity cases. The first correct implementation took 626.5 seconds; the first ambiguity-gated run completed in 305.4 seconds, the authoritative final-tree run completed in 318.9 seconds, and the positive checker fell from about 7.2 seconds to 2.76 seconds without changing the direct five-way probe.

## Surviving Container State And Tab Continuation Addendum

Candidate `c4bfa01` passed all seven hosted jobs in run `29380939276`, but correctness, security, and API/documentation all returned `REVISE`. Exact review proved that implicit inner-container termination discarded a surviving outer list, the ambiguity gate missed an indented quote or list before a fence, and exact continuation width rejected a tab that validly supplied required columns plus relative fence indentation. A retired README URL guard still read raw Markdown, and the debug next action self-staled after candidate creation.
