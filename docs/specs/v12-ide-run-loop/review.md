...
Candidate-addressed external evidence records that Revision 61 passed exact committed and hosted checks but is retired after its non-consuming exact-candidate rehearsal reported a timeout for the positive copied gate at the old 15-minute bound; its one-shot gate remains unused. Revision 62 scopes a 30-minute exception to that one positive operation, preserves ordinary timeout and process-tree policy, and passes the exact hook, 66 focused release tests, broad pre-freeze verification, and independent review. Exact committed, hosted, canonical, fresh R2, closeout, and merge gates remain. `releaseReady: false`.
