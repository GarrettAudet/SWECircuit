...
Candidate-addressed external evidence records that Revision 62 passed its corrected exact lifecycle, then failed the complete verifier and all seven hosted jobs because its R62-only binary attribute changed V11's authenticated `.gitattributes` identity; its one-shot gate remains unused and the source is retired. Revision 63 removes only that attribute, preserves the LF-only R61 evidence bytes, keeps V11-approved source bytes unchanged, and passes strict V11 dogfood, 68 focused release tests, 465 core tests, complete pre-freeze verification, the checker matrix, and independent review. Exact committed, hosted, canonical, fresh R2, closeout, and merge gates remain. `releaseReady: false`.
