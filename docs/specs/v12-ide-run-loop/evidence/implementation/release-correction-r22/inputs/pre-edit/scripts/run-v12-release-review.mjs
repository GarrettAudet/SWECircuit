...
function environmentPolicy(inherited) {
  return {
    ...
    npmUserConfig: "disabled-null-device",
    npmGlobalConfig: "disabled-null-device",
    npmAudit: "disabled",
    npmFunding: "disabled",
...
function closedEnvironment(tools, cache, additions = {}) {
...
async function resolveHostTools(cache) {
  ...
  const environment = closedEnvironment(basic, cache);
  const gitVersion = strictUtf8(
    requireRun(run(gitPath, ["--version"], { environment }), "Git version inspection").stdout,
    "Git version",
  ).trim();
  const npmVersion = strictUtf8(
    requireRun(run(nodePath, [npmPath, "--version"], { environment }), "npm version inspection").stdout,
    "npm version",
  ).trim();
  return {
    node: { ...(await regularFileBinding(nodePath, "Node executable")), version: process.version },
    git: { ...(await regularFileBinding(gitPath, "Git executable")), version: gitVersion },
    npm: { ...(await regularFileBinding(npmPath, "npm CLI")), version: npmVersion },
    cache,
  };
}
...
function gitResult(tools, arguments_, options = {}) {
  return run(tools.git.path, arguments_, {
    cwd: options.cwd ?? ROOT,
    environment: options.environment ?? closedEnvironment(tools, tools.cache),
  });
}
...
async function installExactSupply(root, tools, cache, lockSupply, packageBytes) {
  ...
  const environment = closedEnvironment(tools, cache);
  const result = run(
    tools.node.path,
...
async function buildCandidate(root, tools, cache) {
  ...
  const result = run(tools.node.path, [entrypoint, "-p", "tsconfig.json"], {
    cwd: root,
    environment: closedEnvironment(tools, cache),
  });
...
async function createGitContext(operationRoot, candidateRoot, checkpoint, tools, cache) {
  const base = closedEnvironment(tools, cache);
  ...
  requireRun(run(tools.git.path, ["clone", "--bare", "--shared", "--quiet", "--", common, gitRoot], { environment: base }), "Disposable Git clone");
  const environment = closedEnvironment(tools, cache, {
  ...
  for (const [label, arguments_] of [
    ...
    requireRun(run(tools.git.path, arguments_, { cwd: candidateRoot, environment: setup }), label);
  }
...
async function execute() {
  ...
  try {
    ...
    const tools = await resolveHostTools(cache);
