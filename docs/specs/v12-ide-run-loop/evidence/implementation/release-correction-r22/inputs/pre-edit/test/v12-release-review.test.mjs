...
test("isolated copied production entrypoints complete one exact compile-to-verify lifecycle", {
  ...
  for (const [path, expected] of Object.entries(R21_FROZEN_IDENTITIES)) {
  ...
  try {
    ...
    assert.ok(
      parentSource.indexOf("validateCacheLocation(cache, [ROOT, operationRoot]);") <
        parentSource.indexOf("const tools = await resolveHostTools(cache);"),
      "cache overlap must be rejected before cache-using tool inspection",
    );
