...
const R21_ROOT = "docs/specs/v12-ide-run-loop/evidence/implementation/release-correction-r21";
...
export const R21_FROZEN_IDENTITIES = Object.freeze({
...
async function initializeFixtureGit(fixtureRoot) {
  ...
  await runGit(
    fixtureRoot,
    ["commit", "--no-gpg-sign", "-m", "Revision 21 isolated production lifecycle"],
    { env: gitEnvironment },
  );
...
async function authenticateFixtureBlobs(fixtureRoot, commit, expected) {
  ...
  return { rows, treeFiles: treePaths.length, exclusions: [...COPY_EXCLUSIONS] };
}
...
async function createNpmCompatibilityAdapter(lifecycleRoot) {
  const hostNpmCli = join(dirname(process.execPath), "node_modules", "npm", "bin", "npm-cli.js");
  const hostManifest = JSON.parse(
    await readFile(join(dirname(process.execPath), "node_modules", "npm", "package.json"), "utf8"),
  );
  const adapterPath = join(lifecycleRoot, "npm-cli-adapter.mjs");
  const adapterBytes = Buffer.from(
    [
      'import { spawnSync } from "node:child_process";',
      `const hostNpmCli = ${JSON.stringify(hostNpmCli)};`,
      "const environment = { ...process.env };",
      "for (const key of Object.keys(environment)) {",
      '  if (key.toLowerCase() === "npm_config_globalconfig") delete environment[key];',
      "}",
      "const result = spawnSync(process.execPath, [hostNpmCli, ...process.argv.slice(2)], {",
      "  env: environment,",
      '  stdio: "inherit",',
      "  windowsHide: true,",
      "});",
  ...
  await writeFile(adapterPath, adapterBytes, { flag: "wx" });
  return {
    path: adapterPath,
    identity: { bytes: adapterBytes.byteLength, digest: digest(adapterBytes) },
    hostNpmCli,
    hostNpmIdentity: await identity(hostNpmCli),
    hostNpmVersion: hostManifest.version,
    reason:
      "npm 11 rejects the frozen parent's duplicate NUL user/global config source; the external launcher removes only globalconfig and delegates exact arguments to the installed npm CLI.",
  };
}

function parentEnvironment(cacheRoot, parentDigest, npmAdapterPath, gitExecutablePath) {
  const environment = { ...process.env };
  ...
  setEnvironmentValue(environment, "npm_execpath", npmAdapterPath);
  setEnvironmentValue(
    environment,
    "PATH",
...
export async function runReleaseReviewProductionLifecycle() {
  ...
  const sourceFrozenBefore = await authenticateFiles(SOURCE_ROOT, R21_FROZEN_IDENTITIES);
  assertIdentity(await identity(absolute(SOURCE_ROOT, LOCK_PATH)), LOCK_IDENTITY, LOCK_PATH);

  const lifecycleRoot = await realpath(
    await mkdtemp(join(resolve(tmpdir()), "swecircuit-r21-lifecycle-")),
  );
  ...
  try {
    ...
    await cp(absolute(SOURCE_ROOT, ".local/npm-cache"), cacheRoot, { recursive: true });
    const npmAdapter = await createNpmCompatibilityAdapter(lifecycleRoot);
    const gitSupply = await resolveHostGitSupply(lifecycleRoot);
    assert.equal(await pathExists(absolute(fixtureRoot, R21_ROOT)), false);
    ...
    const copiedProduction = await authenticateFiles(fixtureRoot, R21_FROZEN_IDENTITIES);
    assertIdentity(
    ...
    const parentDigest = R21_FROZEN_IDENTITIES[PARENT_PATH].digest;
    const environment = parentEnvironment(cacheRoot, parentDigest, npmAdapter.path, gitSupply.path);
    ...
    result = {
      ...
      fixture: {
        repositoryWasOutsideSource: !resolve(fixtureRoot).startsWith(`${resolve(SOURCE_ROOT)}\\`),
        commit,
    ...
    cleanup.sourceFrozenAfter = await authenticateFiles(SOURCE_ROOT, R21_FROZEN_IDENTITIES);
