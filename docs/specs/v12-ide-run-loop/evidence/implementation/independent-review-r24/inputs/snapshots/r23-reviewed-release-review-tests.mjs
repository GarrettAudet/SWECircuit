...
test("copied production lifecycle uses the release-gate host npm cache supply", async () => {
  assert.equal(
    V12_RELEASE_REVIEW_LIFECYCLE_TEST_HOOKS.hostNpmCache,
  ...
  try {
    await mkdir(source);
    await writeFile(join(source, "sentinel.txt"), "cache supply\n", "utf8");
    const evidence = await V12_RELEASE_REVIEW_LIFECYCLE_TEST_HOOKS.copyHostNpmCacheSupply(
      destination,
      source,
    );
    assert.equal(evidence.sourceSelection, "release-gate-host-npm-cache");
    assert.equal(evidence.rootsDisjoint, true);
    assert.equal(readFileSync(join(destination, "sentinel.txt"), "utf8"), "cache supply\n");
    await assert.rejects(
      () =>
        V12_RELEASE_REVIEW_LIFECYCLE_TEST_HOOKS.copyHostNpmCacheSupply(
          join(source, "nested-copy"),
          source,
        ),
      /overlaps the lifecycle-owned copy/u,
    );
  } finally {
