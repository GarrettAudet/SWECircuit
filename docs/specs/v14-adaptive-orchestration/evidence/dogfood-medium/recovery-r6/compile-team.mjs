...
const SOURCE_DEFINITIONS = [
  {
    ...
    path: "docs/specs/v14-adaptive-orchestration/evidence/dogfood-medium/recovery-r6/goal.md",
    description: "Closed revision-6 pointer-target recovery objective and acceptance criteria.",
  },
  {
    id: "context.predecessor",
    path: "docs/specs/v14-adaptive-orchestration/evidence/dogfood-medium/recovery-r6/predecessor.json",
    description: "Exact revision-5 fix route, pointer-target finding, repair, and lineage identities.",
  },
  {
  ...
  {
    id: "context.rebound-verification-handoff",
    path: "docs/specs/v14-adaptive-orchestration/evidence/dogfood-medium/recovery-r5/handoffs/verification-raw.json",
    description: "Verified revision-5 package and product verification PASS handoff.",
  },
  {
    id: "context.pointer-target-review-handoff",
    path: "docs/specs/v14-adaptive-orchestration/evidence/dogfood-medium/recovery-r5/handoffs/review-raw.json",
    description: "Verified revision-5 pointer-target review FIX handoff.",
  },
...
async function buildRequest() {
  ...
  return {
    ...
    goal: {
      ...
      revision: 6,
      objective:
        "Bind the full-row pointer-target correction, verify the complete Impact Planner in the browser, and obtain fresh independent approval without further application changes.",
      integrationOwner: "codex.main",
      assumptions: [
        {
          id: "assumption.bounded-pointer-target-repair",
          statement:
            "The revision-6 candidate changes only the changed-component label styling and its focused regression assertion after the verified revision-5 review finding.",
          rationale:
            "The full 44px label now owns checkbox activation, and the local suite, syntax checks, desktop/mobile geometry, padding click, overflow, and console checks passed before freezing the package.",
        },
      ],
      unresolvedDecisions: [],
      acceptanceCriteria: [
        ...
        },
        {
          ...
          description:
            "The exact corrected Impact Planner remains functionally and securely complete, and the full changed-component label is an interactive target at least 44px high.",
          evidenceRequirements: [
            evidence(
