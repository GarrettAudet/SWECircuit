...
const CODEC_UNIT = "implement.impact-codec-storage";
const CODEC_CAPABILITY = "implement.safe-workspace-codec";
const INTERFACE_UNIT = "implement.impact-interface";
const INTERFACE_CAPABILITY = "implement.accessible-impact-interface";
...
async function main() {
  ...
  const allCapabilities = [GRAPH, CODEC_CAPABILITY, INTERFACE_CAPABILITY, INTEGRATION, REVIEW];
  const caps = (qualityById) =>
    allCapabilities.map((id) => capability(id, qualityById[id] ?? low));
  ...
  const runtimeRows = [
    runtimeRow({
      ...
      capabilities: caps({ [INTERFACE_CAPABILITY]: balanced }),
      tools: ["node", "rg"],
      costRank: 0,
    ...
    runtimeRow({
      ...
      capabilities: caps({
        [GRAPH]: balanced,
        [CODEC_CAPABILITY]: balanced,
        [INTERFACE_CAPABILITY]: balanced,
    ...
    runtimeRow({
      ...
      capabilities: caps({
        [GRAPH]: frontier,
        [CODEC_CAPABILITY]: frontier,
        [INTERFACE_CAPABILITY]: frontier,
        [INTEGRATION]: balanced,
        [REVIEW]: balanced,
      }),
  ...
  const request = {
    ...
    policy: {
      ...
      capabilityRules: [
        ...
        capabilityRule({
          capabilityId: CODEC_CAPABILITY,
          quality: balanced,
          reasoning: "reasoning.medium",
          tools: ["node", "rg"],
        }),
        capabilityRule({
          capabilityId: INTERFACE_CAPABILITY,
          quality: balanced,
          reasoning: "reasoning.medium",
          tools: ["node", "rg"],
        }),
    ...
    calibration: {
      ...
      id: "calibration.codex-desktop.impact-planner",
      revision: 1,
    ...
    inventory: {
      ...
      id: "inventory.codex-desktop.impact-planner",
      revision: 1,
  ...
  const assignmentCompilation = requireValue(
    "compile Impact Planner runtime assignments",
    compileRuntimeAssignments(request),
  );
  requireValue(
    "verify Impact Planner runtime assignments",
  ...
  const expected = new Map([
    [GRAPH, ["profile.codex.terra", "effort.high"]],
    [CODEC_UNIT, ["profile.codex.terra", "effort.medium"]],
    [INTERFACE_UNIT, ["profile.codex.luna", "effort.medium"]],
  ...
  for (const [workUnitId, [profileId, effortId]] of expected) {
    ...
    if (
      ...
      assignment.effortId !== effortId
    ) {
      throw new Error(
        `Unexpected ${workUnitId} assignment: ${JSON.stringify(assignment)}`,
      );
    }
  }
  if (
