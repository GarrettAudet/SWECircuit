...
Revision 24 is in release hardening. Revision 23 is retired before integration after independent security/trace review found that token-local secret checks allowed credential-assignment and Bearer-style values split across JSON Pointer delimiters to survive in public diagnostics. The corrected public diagnostic boundary, candidate-analysis API, closed raw-handoff verification and fan-in API, truthful first-run example, and installed-package compatibility gate are implemented and pass focused verification. Fresh revision-24 source bindings, packages, approvals, Audit B, Candidate A review/release, integration replay, canonical gates, and owner approval remain required.
...
- AC14-AC16 have focused implementation evidence but remain open until revision 24 binds the current sources, repeats the relevant suites through the release specialist, and receives independent review.
