...
Revision 27 is in release hardening. Revision 26 produced reproducible, approval-verified Candidate A and Audit B packages and a passing external receipt, but its Audit B binder returned a structurally invalid public handoff because the generated agent contract exposed only top-level field names, not the closed nested envelope. The public verifier rejected it with `SC4310`; zero results were accepted, Candidate A was never authorized, and no integration occurred. The full checker matrix also exposed one stale fixture expectation. Both causes are corrected in the current source, and fresh revision-27 bindings, packages, approvals, independent reviews, integration replay, canonical gates, and owner approval remain required.
...
Revision 6 remains historical evidence for the earlier implementation, not acceptance of the current tree. Revisions 17-26 are preserved under `evidence/dogfood/runs/`. Revision 24 stopped before launch on a contradictory prelaunch handoff contract. Revision 25 corrected that contract but stopped on a concise-README compatibility regression. Revision 26 restored the public contract and reached an authenticated Audit B binder; its raw result then failed the strict handoff verifier because generated instructions did not define the nested envelope, while the checker matrix identified one stale expected diagnostic. All revision-26 identities and evidence are retired.
...
- AC11 remains open for the current tree until the exact revision-27 release specialist repeats authority-contained canonical verification and the full negative checker matrix against immutable source bindings.
- AC12 remains open until revision 27 completes both approvals, the package-verification receipt, Audit B, Candidate A specialist execution, integration, post-integration reconstruction, and preserved raw handoffs.
- AC13 remains open until all independent revision-27 review lanes return `PASS` against one exact digest pair.
- AC14-AC18 have focused implementation evidence but remain open until revision 27 binds the current sources, repeats the relevant suites through the release specialist, and receives independent review.
- AC19 has a passing renderer regression and remains open until a fresh revision-27 binder returns a raw public handoff accepted from the generated contract.
