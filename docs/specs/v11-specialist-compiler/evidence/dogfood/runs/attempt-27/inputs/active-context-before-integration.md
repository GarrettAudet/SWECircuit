...
Complete V11 Specialist Compiler revision 27 without widening its pure-core boundary. Revision 26 restored the public README contract and produced reproducible Candidate A and Audit B packages plus a passing external receipt. Its authenticated Audit B binder returned a structurally invalid public handoff because the generated contract did not define the closed nested envelope; `verifySpecialistHandoff` rejected it with `SC4310`. The checker matrix also exposed one stale fixture expectation. Zero results were accepted, Candidate A was never authorized, and no integration occurred. Both causes are corrected. Current work is exact source freeze, fresh packages and approvals, independent execution, verified fan-in, integration replay, canonical gates, and milestone acceptance.
...
`safePointer` validates each candidate emitted pointer prefix and its decoded semantic prefix before appending a token, preserving only the longest prefix that does not match the shared high-confidence secret detector. Revisions 24-26 then added candidate analysis, strict raw handoffs, transitive fan-in, a truthful first run, packed-consumer parity, the corrected prelaunch superset, restored public compatibility, and exact evidence for the generated handoff instruction gap. Revision 27 must bind all current bytes and repeat the complete two-package audit and Candidate A cycle. No hosted CI, merge, provider execution, or V11 runtime enforcement is claimed.
...
Revision 27 renders one concrete verifier-valid `SpecialistAgentHandoff` example in every generated contract and tests every nested shape through the public API. The checker fixture now expects the link it actually hides. Pre-freeze format, lint, typecheck, build, 364/364 kernel tests, the template checker, and all 119 checker cases pass. No revision-27 digest pair is approved yet.
...
- Runtime handoffs must be preserved as exact raw UTF-8 bytes, verified against trusted package expectations, and assessed as a complete transitive dependency closure before dependent integration.
- Generated agent contracts must include a concrete closed handoff example with exact nested shapes; top-level required-field lists alone are insufficient runtime instructions.
...
- V11 remains stacked on owner-gated V10 for continuity. Revision-26 independent acceptance, replay, branch publication, baseline selection, and owner merge approval remain open.
...
- Freeze the current feature spec and active context as immutable revision-27 integration inputs and refresh every repository source tuple.
- Generate deterministically reproducible Candidate A and Audit B packages from the corrected source tree.
- Apply the standing owner approval to the exact Audit B pair, create and bind the exact `PrelaunchPackageVerificationReceipt`, run the binder and semantic reviewer, then preserve the strict `PrelaunchAuditHandoff` and cross-package authorization.
- Apply the standing owner approval to Candidate A only after Audit B `PASS`, then launch only its exact verified contracts.
- Require preparation, product/API, algorithm/lifecycle, security/trace, authority-contained release verification, integration-owner review, and post-integration package replay to pass against the exact revision-27 pair before branch freeze.
