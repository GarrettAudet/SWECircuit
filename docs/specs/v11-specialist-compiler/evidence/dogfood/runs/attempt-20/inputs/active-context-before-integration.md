...
Complete V11 Specialist Compiler revision 20 without widening its pure-core boundary. Revision 19 is retired after independent review exposed a final handoff Unicode defect and release-host containment issues; current work must bind those corrections into fresh Candidate A and Audit B packages, approvals, reviews, release evidence, integration replay, and milestone acceptance.
...
The final handoff scanner now rejects U+D800-U+DFFF while preserving valid supplementary characters. Direct reproduction against `verifyLaunchAuthorization` failed before correction and the focused reference-host suite now passes 25/25 across every guarded free-text surface. Revision 20 must run canonical verification with TEMP, TMP, and npm cache resolved inside declared `.local/npm-cache/**` authority and use a verified short-path spelling of that same target for the Windows negative checker matrix. No hosted CI, merge, provider execution, or V11 runtime enforcement is claimed.
...
- Contract and handoff text rejects C0/C1 controls, DEL, malformed Unicode, and Unicode bidirectional formatting controls before rendering.
...
- V11 remains stacked on owner-gated V10 for continuity. Revision-20 technical acceptance, replay, branch publication, baseline selection, and owner merge approval are all still open.
...
- Freeze the current feature spec and active context as immutable revision-20 integration inputs and refresh all 40 repository source tuples.
