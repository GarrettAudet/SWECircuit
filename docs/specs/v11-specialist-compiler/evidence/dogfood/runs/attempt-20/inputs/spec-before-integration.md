...
Reopened during production-hardening dogfood. Revision 19 is retired after independent security review found that final launch authorization accepted escaped lone UTF-16 surrogates; release verification separately found undeclared temporary-workspace use and a Windows path-length failure. The causal Unicode correction and focused 25/25 reference-host suite pass. Fresh revision-20 source bindings, packages, approvals, Audit B, Candidate A review/release, integration replay, and owner approval remain required.
...
Revision 6 remains historical evidence for the earlier implementation, not acceptance of the current tree. Revisions 17-19 are preserved under `evidence/dogfood/runs/`. Revision 19 ended with security `REVISE` and release `FIX` before integration; every earlier Candidate A and Audit B pair is retired.

- AC1-AC10 current pre-candidate evidence: revision 19 demonstrated the complete two-package launch gate and independent review fan-out, then retired correctly on security `REVISE`. The lone-surrogate correction builds and the verifier-level host suite passes 25/25; canonical gates must replay against revision-20 bindings.
- AC11 remains open until the exact revision-20 release specialist completes authority-contained canonical verification and the full negative checker matrix against immutable source bindings.
- AC12 remains open until revision 20 completes both approvals, the package-verification receipt, Audit B, Candidate A specialist execution, and preserved raw handoffs.
- AC13 remains open until all independent revision-20 review lanes return `PASS` against one exact digest pair.
