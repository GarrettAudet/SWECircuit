...
Revision 25 is in release hardening. Revision 24 was retired before agent launch when prelaunch inspection found that Audit B's custom semantic envelope could not satisfy the compiled blueprint's standard `agent` and `compilationDigest` handoff fields without being rejected as an unknown-field shape. The closed superset correction and focused prelaunch regressions pass. Fresh revision-25 source bindings, packages, approvals, Audit B, Candidate A review/release, integration replay, canonical gates, and owner approval remain required.
...
- AC14-AC17 have focused implementation evidence but remain open until revision 25 binds the current sources, repeats the relevant suites through the release specialist, and receives independent review.
