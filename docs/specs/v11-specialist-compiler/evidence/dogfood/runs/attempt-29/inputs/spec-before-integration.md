...
Revision 29 release hardening is active. Revision 28 passed separate Candidate A and Audit B approval, the independent prelaunch audit, candidate preparation, and algorithm/lifecycle plus security/trace review. Product/API and release verification independently returned `FIX` because the checked-in first-run approval retained an older contract byte count and digest. Machine fan-in received every dependency but correctly reported `integrationReady: false`; no integration occurred. The exact run is retired under `evidence/dogfood/runs/attempt-28/`. The approval now binds current source bytes and the newly derived compilation/package pair, and both focused first-run tests pass. Fresh revision-29 packages, approvals, independent reviews, full release verification, integration replay, and owner approval remain required.
...
Revision 6 remains historical evidence for the earlier implementation, not acceptance of the current tree. Revisions 17-28 are preserved under `evidence/dogfood/runs/`. Revision 28 proved the complete two-package admission path and launched the compiled Candidate A fan-out. Preparation, algorithm/lifecycle, and security/trace passed; product/API and release independently found the same stale first-run approval. All five Candidate A results pass public structural verification, but machine fan-in records `integrationReady: false`; every revision-28 identity and approval is retired.
...
- AC11 remains open for the current tree until the exact revision-29 release specialist repeats authority-contained canonical verification and the full negative checker matrix against immutable source bindings.
- AC12 remains open until revision 29 completes both approvals, the package-verification receipt, Audit B, Candidate A specialist execution, integration, post-integration reconstruction, and preserved raw handoffs.
- AC13 remains open until all independent revision-29 review lanes return `PASS` against one exact digest pair.
- AC14-AC22 have focused implementation evidence but remain open until revision 29 binds the current sources, repeats the relevant suites through the release specialist, and receives independent review.
- AC23 has a corrected current approval and 2/2 focused first-run tests; it remains open until the exact revision-29 release specialist passes the complete canonical gate.
