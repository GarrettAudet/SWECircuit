...
Revision 28 release hardening is active. Revision 27 completed separate Candidate A and Audit B approval, independent prelaunch audit, candidate preparation, and algorithm/lifecycle review. Product/API, security/trace, and release verification then returned exact public `FIX` handoffs for an unexported common-schema dependency, rendering-unsafe handoff controls and media-type substitution, and one stale packed-consumer approval. Machine fan-in was complete but `integrationReady: false`; no integration occurred. The exact run is retired under `evidence/dogfood/runs/attempt-27/`. The three causal corrections pass 54/54 focused tests and the clean offline installed consumer; fresh revision-28 bindings, packages, approvals, independent reviews, integration replay, canonical gates, and owner approval remain required.
...
Revision 6 remains historical evidence for the earlier implementation, not acceptance of the current tree. Revisions 17-27 are preserved under `evidence/dogfood/runs/`. Revision 27 proved the full two-package prelaunch path and launched the compiled Candidate A fan-out, then stopped before integration when product/API, security/trace, and release verification returned three public `FIX` handoffs. Its five Candidate A results all pass public structural verification, but machine fan-in records `integrationReady: false`; every revision-27 identity and approval is retired.
...
- AC11 remains open for the current tree until the exact revision-28 release specialist repeats authority-contained canonical verification and the full negative checker matrix against immutable source bindings.
- AC12 remains open until revision 28 completes both approvals, the package-verification receipt, Audit B, Candidate A specialist execution, integration, post-integration reconstruction, and preserved raw handoffs.
- AC13 remains open until all independent revision-28 review lanes return `PASS` against one exact digest pair.
- AC14-AC19 have focused implementation evidence but remain open until revision 28 binds the current sources, repeats the relevant suites through the release specialist, and receives independent review.
- AC20-AC22 have focused regression and clean-consumer evidence; they remain open until the exact revision-28 product/API, security/trace, and release specialists independently accept the frozen package.
