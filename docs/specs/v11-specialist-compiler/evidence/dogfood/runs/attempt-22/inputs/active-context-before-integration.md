...
Complete V11 Specialist Compiler revision 22 without widening its pure-core boundary. Revision 21 is retired after independent review exposed a canonical-semantics versus rendered-bytes normalization gap and release verification exposed an unbounded checker-matrix harness. Both causal fixes are source-level green; current work must bind them into fresh Candidate A and Audit B packages, approvals, reviews, release evidence, integration replay, and milestone acceptance.
...
The compiler now explicitly reconstructs every nested closed record before compilation and rendering; a regression compares ordinary JSON serialization, every rendered file, and the package digest under logical key permutations. The checker matrix now excludes generated evidence the checker never reads, uses a fixed four-slot hard-linked fixture pool with atomic copy-on-write mutations, and restores only changed paths. Focused and full source-level gates pass, including 353/353 kernel tests and 119/119 checker cases in 213.3 seconds. Revision 22 must bind these exact bytes and repeat the complete two-package audit and Candidate A cycle. No hosted CI, merge, provider execution, or V11 runtime enforcement is claimed.
...
- V11 remains stacked on owner-gated V10 for continuity. Revision-22 technical acceptance, replay, branch publication, baseline selection, and owner merge approval are all still open.
...
- Freeze the current feature spec and active context as immutable revision-22 integration inputs and refresh every repository source tuple.
...
- Require preparation, product/API, algorithm/lifecycle, security/trace, authority-contained release verification, integration-owner review, and post-integration package replay to pass against the exact revision-22 pair before branch freeze.
