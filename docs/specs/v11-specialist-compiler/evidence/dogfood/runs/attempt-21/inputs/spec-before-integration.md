...
Reopened during production-hardening dogfood. Revision 20 is retired after independent security review found that semantic handoffs accepted high-confidence secrets and escaped TAB/LF/CR metadata, while a variable handoff path lacked an explicit Unicode-scalar gate. Release verification correctly detected the resulting source drift and returned `FIX`; integration did not launch. The causal correction passes 353/353 kernel tests plus format, lint, and typecheck. Fresh revision-21 source bindings, packages, approvals, Audit B, Candidate A review/release, integration replay, and owner approval remain required.
...
Revision 6 remains historical evidence for the earlier implementation, not acceptance of the current tree. Revisions 17-20 are preserved under `evidence/dogfood/runs/`. Revision 20 passed preparation, algorithm/lifecycle, and product/API review, then ended with security `REVISE` and release freshness `FIX` before integration; every earlier Candidate A and Audit B pair is retired.

- AC1-AC10 current pre-candidate evidence: revision 20 demonstrated the complete two-package launch gate, approval-bound fan-out, fail-closed security review, and source-freshness retirement. The three handoff/path corrections pass the focused 65/65 suites and the full 353/353 kernel suite; canonical gates must replay against revision-21 bindings.
- AC11 remains open until the exact revision-21 release specialist completes authority-contained canonical verification and the full negative checker matrix against immutable source bindings.
- AC12 remains open until revision 21 completes both approvals, the package-verification receipt, Audit B, Candidate A specialist execution, and preserved raw handoffs.
- AC13 remains open until all independent revision-21 review lanes return `PASS` against one exact digest pair.
