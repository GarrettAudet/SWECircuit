...
Complete V11 Specialist Compiler revision 23 without widening its pure-core boundary. Revision 22 is retired after independent security review exposed unsafe public diagnostic artifact and pointer sanitization. The causal fix is source-level green; current work must bind it into fresh Candidate A and Audit B packages, approvals, reviews, release evidence, integration replay, and milestone acceptance.
...
`createDiagnostic` now suppresses artifacts containing controls, lone surrogates, or high-confidence secrets, and truncates pointers before any unsafe control, lone surrogate, secret, traversal, slash, backslash, or colon token. Valid supplementary Unicode remains accepted. Public-API, JSON-parser, and contained-read regressions pass; format, lint, typecheck, and the complete kernel pass 354/354. Revision 23 must bind these exact bytes and repeat the complete two-package audit and Candidate A cycle. No hosted CI, merge, provider execution, or V11 runtime enforcement is claimed.
...
- Freeze the current feature spec and active context as immutable revision-23 integration inputs and refresh every repository source tuple.
...
- Require preparation, product/API, algorithm/lifecycle, security/trace, authority-contained release verification, integration-owner review, and post-integration package replay to pass against the exact revision-23 pair before branch freeze.
