...
Reopened during production-hardening dogfood. Revision 22 is retired before integration after independent security/trace review found that public diagnostics preserved lone UTF-16 surrogates in artifact and pointer fields and preserved high-confidence secret text in artifact fields. Audit B, preparation, product/API, algorithm/lifecycle, and release verification passed; the release lane completed 353/353 tests and all 119 checker cases. The diagnostic sanitizer correction now passes focused coverage, format, lint, typecheck, and 354/354 kernel tests. Fresh revision-23 source bindings, packages, approvals, Audit B, Candidate A review/release, integration replay, and owner approval remain required.
...
- R24: Sanitize every public diagnostic artifact and pointer before return: reject controls and lone surrogates in both fields, reject high-confidence secrets in both fields, retain only the longest safe pointer prefix, and preserve valid supplementary Unicode.
...
Revision 6 remains historical evidence for the earlier implementation, not acceptance of the current tree. Revisions 17-22 are preserved under `evidence/dogfood/runs/`. Revision 22 passed Audit B, preparation, product/API, algorithm/lifecycle, and release verification, then ended with security/trace `REVISE` before integration; every earlier Candidate A and Audit B pair is retired.

- AC1-AC10 current pre-candidate evidence: revision 22 demonstrated exact two-package admission and four-lane parallel review against Candidate A `sha256:642ad0726e8019231d6e357a8602c387a47a637d2ce94fc0701efedaf1ae5869` / `sha256:8ddb1130c6af875f708fb347d46473d532dec0ff796834869ee644f6ee488bbc`. Its security finding produced public diagnostic scalar/privacy coverage through `createDiagnostic`, `parseJsonBuffer`, and a contained-read failure; revision 23 must bind those corrected bytes.
- AC11 remains open for the current tree until the exact revision-23 release specialist repeats authority-contained canonical verification and the full negative checker matrix against immutable source bindings.
- AC12 remains open until revision 23 completes both approvals, the package-verification receipt, Audit B, Candidate A specialist execution, integration, post-integration reconstruction, and preserved raw handoffs.
- AC13 remains open until all independent revision-23 review lanes return `PASS` against one exact digest pair.
