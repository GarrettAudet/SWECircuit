...
Ship V11 as the standalone, built-in Specialist Compiler defined by ADR 0004. The owner-facing goal is hyper-optimized task-shaped agent construction: an IDE or human clarifies one software goal into stable atomic work units; core independently constructs legal team partitions, keeps a serial baseline, selects the best eligible roster under a fixed objective, and emits exact digest-bound agent and integration contracts. The larger scheduler/restart/parent-trace/merge control plane is deferred with its failed-review evidence preserved.
...
The active V11 implementation checkpoint is 748b21f on codex/v11-orchestration-planner. It adds ADR 0004, a new feature package, a closed specialist input schema, canonical domain-separated SHA-256 support, stable SC4301-SC4309 diagnostics, deriveTaskAuthorityProjection, compileAgentBlueprints, exhaustive canonical partition search through eight work units, deterministic bounded structural search above eight, hard authority/evidence gates, conflict-aware projected scheduling, exact AgentBlueprint compilation, and renderSpecialistPackage. It does not change the six V9 artifact kinds or V10 execution semantics.

Checkpoint evidence: strict typecheck and build pass; all 275 inherited tests pass; a manual independent-verification goal compiled to the required two-agent partition and rendered two agent contracts plus integration and manifest files. Golden optimization, schema/consumer integration, source hardening, dogfood, independent review, milestone, and owner approval remain open.
...
- V11 remains stacked on owner-gated V10 for continuity and cannot merge until its baseline/approval relationship is explicitly resolved and the V11 acceptance gate passes.
...
- Integrate golden optimization fixtures, strict schema tests, and clean packed-consumer coverage from the three bounded V11 work packets.
- Fix any source defects those specialists reproduce; keep every correction causal and covered.
- Update the active README, ADR index, milestone, feature tasks, and memory to match the standalone compiler boundary.
- Dogfood compileAgentBlueprints on the remaining V11 tasks and preserve the generated package plus observed comparison.
- Run the canonical kernel and workflow gates, freeze one immutable candidate, and fan out product/API, algorithm/lifecycle, and security/trace review.
- Request owner merge approval only after one exact candidate owns all verification and review evidence.
- Keep the deferred runtime control plane and the separate V10 owner gate explicit.